...
def sanitize_tags(text):
    """Convert internal game tags to a safe public format.
    
    <@ba.xxx> → [[xxx]]
    <#ba.xxx> → [[status:xxx]]
    </>       → [[/]]
    """
    text = re.sub(r'<@ba\.(\w+)>', r'[[\1]]', text)
    text = re.sub(r'<#ba\.(\w+)>', r'[[status:\1]]', text)
    ...
    for char_id, char_data in sorted(char_table.items()):
        ...
        for _, _, info, eid in talent_items:
            ...
            if tn:
                ...
                talents.append({
                    'name': strip_tags(tn),
                    'description': sanitize_tags(replace_placeholders(desc, tvalues))
                })
        ...
        for p in bundles:
            ...
            if pn:
                ...
                potentials.append({
                    'name': strip_tags(pn),
                    'description': sanitize_tags(replace_placeholders(desc, pvalues))
                })
        ...
        for sg in skill_groups.values():
            ...
            if skill_name:
                ...
                combat_skills[key] = {
                    ...
                    'description': sanitize_tags(desc).strip()
                }
