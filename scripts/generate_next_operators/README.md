...
- `TimeDilationAction` 中的能力实体查询会严格保留 owner-spawned 身份、可选 GameplayTag 查询或命名 Context 身份。全局/终结技 `ignoreTargets` 已生成正式 DSL，并在执行时把查询到的稳定实体加入排除集合；这在能力实体寿命开始消费时间倍率后是必要语义。Entity `effectTargets` 的 DSL/运行时查询与寿命效果也已存在，但生成器仍明确拒绝输出，因为子 SkillData 动作尚未从父技能绝对帧迁移到实体时钟。Tangtang、Yvonne、Li Zhiyan、Liino 的连携技因此继续暴露同一调度缺口，不能把该字段误认成纯表现 `EffectAction` 后忽略。
