...
> 主动全可编译干员与完整定义均为 5 名。Wulfgard 的两级天赋附着 Buff 输入、能力实体子技能伤害
> 来源和公共燃烧均已闭合。`takeAtkSnapshot` 在动作 Reset 阶段按技能施法/Buff 实例冻结攻击计算，
> 按动作实例隔离并在周期触发间复用；不能回退为每跳实时攻击面板。完整生成含 9 技能、2 天赋、
> 5 潜能、1 能力实体、2 私有 Buff 和 7 公共 Buff，固定 1.4.4 来源的 `--complete --check` 通过。
