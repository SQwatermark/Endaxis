...
- `HealAction` 当前严格接受 `Normal + ActionSource + MultiplyAttributeCalculation(AttackerOrHealer)`，保留四维属性、倍率、加值和原生治疗标签。队伍目标只接受已证明的 `MainCharacterValidator`、按当前生命比例升序取一个目标，以及在该排序前精确排除主控的三种结构，分别生成 `controlledOperator`、`lowestHealthRatioOperator` 与 `lowestHealthRatioOperatorExceptControlled`。标准场景按执行帧解析主控，并以场景干员顺序作为相同比例时的确定性投影；这只是 Endaxis 简化模型的稳定选择，不宣称是原生同值排序规则。满血不会抑制治疗执行或原始治疗回执。
