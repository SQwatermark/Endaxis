...
- `TimeDilationAction.effectTargets` 中的能力实体查询现在会严格保留 owner-spawned 身份和可选 GameplayTag 查询，不再把整项技能挡在源解析阶段；正式 DSL 仍明确拒绝，因为逻辑实体独立时钟及其对子 SkillData 调度的影响尚未闭环。Tangtang、Yvonne、Li Zhiyan、Liino 的连携技因此统一暴露同一运行时缺口，不能把该字段误认成纯表现 `EffectAction` 后忽略。
