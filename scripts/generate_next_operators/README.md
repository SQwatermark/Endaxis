...
- 命名目标组不会按 `tar`、`smart_target` 等字符串猜测语义。生成中间层会严格记录 `FindTargetAction`、`ContinuousFindTargetAction` 和 `MergeTargetAction` 的帧区间、原生动作顺序、分支路径、选择器类型及合并输入；新增查找器、校验器、后处理器或字段形状会立即报错。该目录目前只是来源证据，只有能够证明写入动作在条件读取前发生且控制流支配读取点时，才允许进一步归约 `Context` 实体数量条件。
...
- `IfElseAction.conditionAction` 已能生成完整成功/失败分支。直接位于 `SequenceAction` 中的条件则会以返回值截断同一序列尾部，不能拆成彼此独立的 `scheduled` 项；当前已把主控身份、距离和目标身份条件显式列为序列守卫。统一调度完成序列边界建模前，这类条件不得被视为已经闭环，也不能因为同类条件已能在 `IfElseAction` 中编译就从根动作覆盖检查中消失。
...
- `CreateBuffAction` 审计层现已保留原生目标来源、目标组、创建次数、Buff 来源和是否继承施法信息。正式 DSL 只接受字面量次数 1、`ActionSource` 来源及已闭环的单敌人/施法者目标。技能运行时会为每次实际启动分配单场唯一施法序号，并在步骤执行时连同来源技能和当时已产生的未返还技力消耗复制给 Buff；该身份不进入项目存档。
...
- 单敌人解析器只会展开形状已经严格验证的 `ForEachAction`，以及 `maxCountPerTarget=1` 的 `ChannelingAction`；其他逐目标或多次引导配置立即报错。若条件树的每条叶子都按相同顺序生成同一批能力实体，且其他叶子动作只把 `target_in_range` 赋值为 `1`，清单可用 `collapseSingleEnemyAbilityEntityBranches` 显式声明按固定单敌人模型折叠；完整条件树仍保留在审计输出中。
