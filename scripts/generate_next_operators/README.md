...
- `<slug>.skills.generated.ts`：编译后的 `SkillDefinition` 列表。
- `<slug>.operator.generated.ts`：可直接供正式入口使用的完整 `OperatorDefinition`。
...
- 投射物飞行时间暂按 `0` 帧处理，并在中间层以 `assumedTravelFrames: 0` 明示；接入 `ProjectileData` 后再替换。
