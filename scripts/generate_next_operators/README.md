...
- `CreateBuffAction` 审计层现已保留原生目标来源、目标组、创建次数、Buff 来源和是否继承施法信息。正式 DSL 只接受字面量次数 1、`ActionSource` 来源及已闭环的单敌人/施法者目标；当前运行时尚无原生施法实例身份，因此 `inheritSourceSkillCastInfo = true` 的真实动作仍会明确阻塞，不能静默丢失该字段后生成。
