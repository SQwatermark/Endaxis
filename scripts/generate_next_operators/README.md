...
- 顶层时间轴动作统一保留原生 `serverActionIndex`。伤害、附着、资源、Buff、黑板动作、投射物和能力实体在跨载体归并时按 `(frame, serverActionIndex)` 排序；递归子技能继承触发它的根动作索引。分支子动作的 `actionIndex` 仍表示分支数组下标，两类索引不可互换。
...
- 递归条件编译骨架能够保持成功/失败分支与嵌套顺序，并生成正式 `branch`/`sequence` DSL；当前接入敌方 Tag Buff 黑板读取/结束、施法者 ID Buff 的检查/结束，以及 `ModifyDynamicBlackboard` 的七种直接值运算。任何未接入的叶子都会报告完整分支路径，整棵条件树不产生输出。
- 庄方宜目前已有三棵顶层条件树能够独立完整编译：普攻二、普攻三的飞剑距离黑板更新，以及普通战技结束上一轮飞剑 Buff。伤害命中和条件根现已保留原生动作顺序；下一步应建立统一的有序调度项，将它们按 `(frame, serverActionIndex)` 合并，再逐项移除技能级 `conditionalActions` 安全门。
