...
- 无筛选 `CharacterTeamFinder` 在固定队伍模型中归约为 `party`；精确 `ExcludeOwnerValidator` 归约为 `partyExceptCaster`，并保留原生队伍逆序迭代。旧式 `FinishBuffAction` 只在 Id 查询、已证明目标组和来源限制形状下进入统一结束步骤。Last Rite 的根级队伍查找由此可结束全队对应普通战技标记；其他 validator、post-processor 或查询组合仍严格阻塞。
...
- `SetSkillCdAtOnce(Reduce, isPercentage=true)` 使用技能配置的基础冷却周期作为比例基数；运行时按明确技能类型/技能身份筛选当前冷却账本。该语义来自原生反编译，不从字段名推断。达坂第一天赋的 `OnOutputDamage -> 减少连携技冷却 -> 消耗准备层 -> 结束自身` 已完整生成。
...
- `unmodeledActionTypes` 是逐技能、临时且公开的转换缺口，不是动作类别白名单。声明值必须确实出现在该技能的 `unresolvedCombatActions` 中，否则生成立即失败；命中的条件叶子只允许从正式 DSL 省略，原始条件树和 `*.audit.json` 的 `unmodeledCombatActions` 仍会保留该事实。若过滤后整棵条件树为空，才可连同无副作用条件省略。当前只用它标记尚未进入正常模拟管线的 `HealAction`，对应技能不得据此宣称治疗行为已完整转换。
