...
- `buffDefinitions` 从直接依赖出发，递归扫描 Buff 时间轴与事件动作创建的间接依赖，并按 Buff ID 去重；循环引用不会造成重复或无限递归，缺失数据源会被明确记录。定义只使用 Buff 自身黑板默认值，并保留原生 GameplayTag ID、时间轴伤害/条件/黑板/资源动作及事件动作；事件只保存创建依赖边，不再嵌套复制子定义。技能应用时传入的覆盖值仍留在 `CreateBuffAction` 载荷中。当前尚未把 Buff 属性和伤害修正器编译成运行时目录。
