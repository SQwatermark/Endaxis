...
- 庄方宜目前已有三棵顶层条件树能够独立完整编译：普攻二、普攻三的飞剑距离黑板更新，以及普通战技结束上一轮飞剑 Buff。它们尚未并入最终技能调度，因为已解析伤害命中还没有保留根动作 `actionIndex`，不能仅按帧猜测同帧伤害、投射物和条件动作的先后。下一步应建立统一的有序调度项，再移除技能级 `conditionalActions` 安全门。
