...
- `AuraAction` 是战斗动作，不是表现占位。当前原始数据中的 22 个光环动作分布在 14 个 SkillData，可向区域内目标施加 Buff，并携带进入光环或离开光环时执行的子动作。生成器会把根技能及已解析能力实体/投射物调用图中的光环结构化为 `auraActions`，保留来源文件、时间区间、范围、目标过滤、Buff 输入和内部动作清单；当前调用图覆盖其中 15 个唯一动作，其余 7 个仍位于尚未接通的条件能力实体等子技能路径。在目标筛选、持续时间和子动作生命周期完整闭环前，所有光环都只参与覆盖审计并阻止对应技能被误报为完整，不生成近似 DSL。
