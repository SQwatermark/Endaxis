...
- 庄方宜目前已有三棵顶层条件树能够独立完整编译：普攻二、普攻三的飞剑距离黑板更新，以及普通战技结束上一轮飞剑 Buff。`resolvedSchedule` 已按 `(frame, actionOrder)` 归并伤害与条件根，`resolvedDamageSequence` 只会在整棵条件树可编译时输出；后续逐项补齐其他根动作类型后再移除对应技能的安全门。
