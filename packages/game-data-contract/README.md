...
`skillSettingResources.ts` 保存已取证的 SkillSetting 资源标量及无外部依赖的严格 JSON 校验。
转换器从 TypeTree 根 float 读取，正式生成文档必须携带 resources；产品装配和候选审计均读取
此段，不再用构造默认值替代资源。附着查表消费者仍可单独消费旧有 data/formulas 子集，
但完整产品入口缺 resources 会失败，不提供1.5秒等兼容回退。

本包是转换器与 Endaxis 交换生成数据时的唯一类型来源，不是原生游戏 schema，也不是模拟运行时。
`buildModifiers.ts` 唯一声明常驻构筑修正成员及属性/倍率分类；`equipment.ts` 保留旧配装
名称的兼容导出，转换器直接引用公共构筑成员。输出保留明确公式槽，来源支持范围仍由
转换分支校验，不能由类型允许受治疗增幅或其他技能冷却推导对应原生转换已经接通。
