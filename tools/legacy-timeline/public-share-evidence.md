...
接入卡蜜拉蝙蝠实体被动并补齐物理异常承受方事件后，未修改输入的当前版本配置截止帧
是 2139：507 条伤害，期望伤害 `3299427.540935281`。跑满 3600 帧为 612 条、
`3464833.3694991497`，末次伤害仍在 2761 帧。完整战斗结果不能覆盖用户保存的截止线。

配置截止内的来源为：弭弗 70 条、`1652702.820948937`；洛茜 381 条、
`1345201.1135436548`；卡蜜拉 38 条、`171664.5538298413`；骏卫 18 条、
...
| ------ | -------: | -------: | -----------: | -------------------: |
| 弭弗   |       69 |       70 |      2236510 | `1585422.9758164021` |
| 洛茜   |      264 |      387 |      2942231 | `1413335.3354000663` |
| 卡蜜拉 |       32 |       38 |       320803 |  `167499.9212168975` |
| 骏卫   |       27 |       13 |       326539 |  `86931.53242726611` |

旧版末次伤害减去准备区后在 70.7 秒，当前在 2131 帧；完整时长末次伤害分别约为
92.45 秒和 2761 帧。这些总量和时刻尚未逐动作归因，而且旧版使用手写技能与装备数据，
...
给物理异常来源施加原生天赋 Buff，并产生新的实体时序，必须重跑全轴，不能把总账简单加 5：修复后为
507 条、`3299427.540935281`，完整 3600 帧为 612 条、`3464833.3694991497`。生产回归现用
两次战技先叠无防备再进入骨折，验证承受方 Before 事件打开连携并额外生成士兵；生成产物回归
同时锁定 Target 编为异常来源。
