...
接入卡蜜拉蝙蝠实体被动后，未修改输入的当前版本配置截止帧是 2139：508 条伤害，
期望伤害 `3253189.7648606286`。跑满 3600 帧为 613 条、`3416589.228109895`，
末次伤害仍在 2761 帧。两个口径相对接入前都只增加四条实体被动追击；完整战斗结果
不能覆盖用户保存的截止线。

配置截止内的来源为：弭弗 70 条、`1585422.9758164021`；洛茜 387 条、
`1413335.3354000663`；卡蜜拉 38 条、`167499.9212168975`；骏卫 13 条、
`86931.53242726611`。当前诊断为可用性 32、连携窗口 6、执行异常 0。卡蜜拉在 360、
...
不能用旧版 27 条对当前 13 条做直接缺失判断。这里不移动公开轴输入、不跳过 HitStop，也
不把旧版消费脆弱规则加回当前实现。
