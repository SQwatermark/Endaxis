...
逐条伤害保留技能类型、伤害类型、法术爆发类型、Buff ID 和动作来源；分账包含零伤害、无
castId 和无来源（null）的公共伤害，不把不同轨道或能力实体猜成同一来源。复合状态等后续伤害
可能没有 castId，应使用 `buffId` 和 `sourceActionId` 继续核对，不能据此断言原施法漏伤害。
`diagnostics.castIssues` 按完整 `castId` 汇总当前原生门禁失败；比较旧新伤害前先排除这些施法，
避免把编辑器为保留放置坐标而强制执行的无效输入误判成技能命中缺失。它只是第一层筛选：后续
无效施法仍可能替换同模板能力实体，截断此前合法施法的长时产物，因此不能把未列入
`castIssues` 自动视为可直接比较。
