...
完整重建在 `operators-and-common-buffs` 阶段联合生成干员与公共 Buff，分两步处理：
先逐人组装，保留最终定义、审计字符串和去重的公共定义，不保留整批原始动作图与完整计划；
再收齐同批定义的读取用途，逐人优化、渲染并释放已处理的定义。
公共定义在优化前严格检查同 ID 冲突，并补齐没有干员直接引用的系统根。
生成与检查各自重新规划全部干员。所有内容渲染成功后才写目录；
第二遍检查同时覆盖干员、审计、公共定义及名称文件，任一失败都会阻止候选发布。
...
每轮生成建立独立的 `OperatorPlanningSources`。固定表共用 32 MiB 的原文缓存，技能、Buff 等
...
- `config/operators.json` 的天赋 `index` 对固定两个槽位是冗余配置，后续应从有证据的
  原生槽位关系直接导出，不把清单数组位置无条件当成原生 index。
- 天赋/潜能的人工 `compile` 标记需要追踪当前实际消费者：已无消费者的删除，仍有消费者
  的改为按原始效果结构走公共编译器，不靠人工名称选择实现。
- 用户要求此项先记录，当前主线是重新设计干员定义编辑器，不继续扩大生成器支线。
  已进行的养成 key 去除不等于上述两项已经完成。
