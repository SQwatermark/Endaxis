...
完整重建在 `operators-and-common-buffs` 阶段联合生成干员与公共 Buff：每名干员规划后，
立即收集其公共定义，只保留去重结果与文件文本，不缓存整批干员计划。
生成与检查各自重新规划全部干员；31名干员合计规划62次，原来的分开生成流程需要124次。
公共定义在优化前严格检查冲突，并补齐没有干员直接引用的系统根。所有内容渲染成功后才写目录；
