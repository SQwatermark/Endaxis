...
describe('共享 BuffFindSettings 来源编码', () => {
  ...
  it.each([-1, 4, '0', 'Unknown', null, undefined])('拒绝未知/强转的查询模式 %j', checkType => {
    expect(() => parseBuffFindSettingsSource({ ...settings, checkType }, 'find')).toThrow('find.checkType');
  });
  ...
  it('Selector PriorityFilter 复用完整查询结构，维持非空 ID 支持边界', () => {
    const priority = {
      ...
      filterType: 'DistanceFromMainCharAsc', onlyReserveMaxPriorityTargets: false,
      limitMaxNum: true, maxNum: 1,
      buffFilterSettings: { buffSettings: { ...settings, buffIdList: ['buff_fixture'] }, buffStackNumType: 'BuffCount' },
    };
    const numeric = structuredClone(priority);
    const nativeSettings = { ...numeric.buffFilterSettings.buffSettings, checkType: 0 };
    const parsePriority = (item: unknown) => parsePriorityFilterSources({ postProcessorData: [item] }, 'filters');
    expect(parsePriority({ ...numeric, buffFilterSettings: { ...numeric.buffFilterSettings, buffSettings: nativeSettings } }))
      .toEqual(parsePriority(priority));
    expect(() => parsePriority({
      ...priority, buffFilterSettings: { buffSettings: settings, buffStackNumType: 'BuffCount' },
...
const convert = {
  ...
  isEnable: true, priorityLevel: 'Default', priorityOffset: 0, serverActionIndex: 1,
  convertFrom: targetFixture('Target'), targetGroupKey: 'center', operationType: 'None',
  translateOperation: 'Rotate180DegAroundRef', translationRef: 'ActionSource', translationDeg: 0,
  excludeTarget: 'InputTarget', blackboardVector3: { x: scalarFixture(0), y: scalarFixture(0), z: scalarFixture(0) },
};
...
describe('目标转换编码与支持子集分离', () => {
  it.each([[0, 'None'], [1, 'ConvertEntityToPosition'], [3, 'ExcludeTarget'], [4, 'ConvertEntityToSlot']])(
    'operationType %s / %s 完整 IR 等价', (number, name) => {
      expect(parseConvert({ operationType: number, translateOperation: 0, translationRef: 0, excludeTarget: 2 }))
        .toEqual(parseConvert({ operationType: name }));
    },
  );

  it('None 分支也保留未消费的旋转/引用字段', () => {
    expect(parseConvert({ translateOperation: 1, translationRef: 4, excludeTarget: 1, translationDeg: 60 }))
      .toEqual(parseConvert({ translateOperation: 'RotateAroundRefCW', translationRef: 'ContextTarget', excludeTarget: 'ActionOwner', translationDeg: 60 }));
  });

  it.each([[2, 'TranslatePosition'], [5, 'ConvertSlotToPosition'], [6, 'ConvertSlotToTarget'], [7, 'ConvertBlackboardValueToPosition']])(
    '已知枚举 %s / %s 仍不擅自放开空间行为', (number, name) => {
      expect(() => parseConvert({ operationType: number })).toThrow(`unsupported operation "${name}"`);
      expect(() => parseConvert({ operationType: name })).toThrow(`unsupported operation "${name}"`);
    },
  );

  it.each([
    { operationType: 8 }, { operationType: '0' }, { translateOperation: 2 },
    { translationRef: 5 }, { excludeTarget: '2' },
