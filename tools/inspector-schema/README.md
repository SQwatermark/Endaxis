...
EquipmentEventHandlerDefinition 与 EquipmentContributionDefinition 按成员提取，递归图成员标为不可展开，不因其存在而放弃普通字段。事件 key、可选优先级和原生 abilityEvent 使用公共字段及句柄；黑板只在贡献根编辑，初始化和响应共享，不再提供事件级黑板。黑板缺省时显示空字典，删除最后一项通过父字段写入规则省略属性；新增和逐级编辑使用公共字典/等级值控件。priority 使用公共可选开关。事件族互斥关系保持，条件/序列仍只在图中编辑。测试覆盖枚举、互斥字段、等级数组、最后一项删除以及字面键路径；尚未进行浏览器视觉验收。
