...
2026-09-06 全方向运行时矩阵（elementalPresentationMatrix.test.ts）：

- 控制输入时刻的测试干员，使用正式模拟服务、Buff 定义和反应配方，不代替全干员技能测试。
- 12 个反应方向均存在实际可见输出及精确转换连接，覆盖输入瞬时图标、输出图标/名称/颜色。
  当前标准配方没有复现隐藏输出缺口；不要因此启动泛化的实例谱系开发。
- 四种附着各验证五次施加产生 1/2/3/4/4 层，随后自然结束；到期后再次施加为新实例，
  不连接前后两段。
...
覆盖叠层、封顶刷新和爆发，佩丽卡→狼卫覆盖转换。深浅主题均截图查看，分时截图
确认光点位置变化。发现燃烧颜色遗漏并修正：附着使用现有 role.element 派生颜色，
异常使用原生 abnormalColorType；不再枚举少量 Buff ID。特别注意附着的原生
abnormalColorType 为 Physical，不能直接拿该字段推断附着元素。补充四种实际附着
定义及四种异常颜色测试；不将组件测试页等同于完整时间轴交互验收。
