...
异类附着的 12 个 `try_*` 包装 Buff 已投影为独立的复合状态工厂目录。每条配方保存元素顺序、按消耗层数执行的 SkillSetting 查表、Blackboard 传递和紧接着创建的 Buff ID，不把工厂误写成最终状态。九条配方直接创建最终状态；新附着为自然元素的三条配方先创建额外 wrapper，再按自然异常层数继续分派。当前只完成严格数据边界，尚未执行配方；必须先接入 SkillSetting 求值端口并投影自然 wrapper，才能替换运行时的显式拒绝。
...
1. 接入复合状态工厂的 SkillSetting 求值，并投影三种自然 wrapper 与 12 种最终状态；
