...
23 个套装使用单一套装效果结构：同一套装装备数达到 3 件及以上时，`collect.ts` 会激活该套装的 `effects` 和 `triggers`。
...
| 轻超域     | `aethertech`（散件目录 `thertech`） | 7      |
| 集成重型   | `aic-heavy`                         | 4      |
| 集成轻型   | `aic-light`                         | 4      |
| 重装信使   | `armored-msgr`                      | 9      |
| 碾骨       | `bonekrusha`                        | 14     |
...
4. **套装协同**：同一套装装备数达到 3 件及以上时激活整套效果
