...
Endaxis 收录了 71 把武器的完整数据，覆盖 5 种武器类型和 4 个稀有度等级。
...
| 长柄武器 | Polearm | `polearm` | 10 | 1 | 2 | 3 | 4 |
...
| `rarity` | 稀有度（3-6） |
| `type` | 武器类型 |
| `baseAtk` | 基础攻击力（9 个精炼等级：Lv1-90） |
| `skill1` | 通用属性 1（9 个精炼等级） |
| `skill2` | 通用属性 2（9 个精炼等级） |
| `skill3` | 专属武器技（含 effects + triggers） |

### 精炼等级

武器技能有 9 个精炼等级，对应干员等级 1-90，每级属性递增。
