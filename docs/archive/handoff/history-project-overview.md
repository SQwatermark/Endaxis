...
本文不再维护，保留用于追溯旧工具链和文档引用。
开发从 [文档总览](../../README.md) 开始，接续工作只读 [当前交接](../../handoff/current-context.md)。
...
2026-09-11本批开发与提交的接续入口是[验收检查点](2026-09-11-acceptance-checkpoint.md)：
正式回调宿主、资源边界和伊冯配置化放置已落地；整体行为验收尚未结束。
枚举式AST门禁已取消，不再列为待办。详细逐轮日志见current-context。

**新会话先核对实际机器和工作树，不从 Git 文档猜测本机环境。** 仓库只记录跨机器共享的
项目事实、证据和设计；绝对路径、工作树清单、当前进程、地址与连接方式保存在被 Git 忽略的
`.local/` 中。规则和模板见[本地环境文档边界](../../development/environment-template.md)。
...
**当前收束入口（2026-09-09）：**先读
[实体能力系统事件统一：已办与待办](2026-09-09-event-unification-checklist.md)，
再读 [事件响应架构](../next/event-response-architecture.md)。主要运行链路已完成统一，
生成器路由与兼容边界仍有待办；不是“尚未开始”，也不是全量迁移完成。

原始优先级决定见
[统一事件监听链路](2026-09-09-event-unification-priority.md)。先审查并消除按监听宿主
重复实现事件的架构问题，再恢复下述真实轴扩展与伤害显示工作；当前进度以收束清单为准。
...
接续当前编辑器工作时，先读 [当前任务快照](../../handoff/current-context.md) 的晚间检查点，再读
...
接续统一 TS 编译器时，先读 [当前任务快照](../../handoff/current-context.md)。需要追溯标签、倒地和余烬阶段的
历史决策时，再读 [2026-08-28 同步交接检查点](2026-08-28-sync-checkpoint.md)；其中的覆盖数字、
分支和“下一步”均是历史快照，不能覆盖当前任务快照。

1. [背景、目标与历史](../../README.md)：项目为何存在，旧版 Endaxis 有什么问题，Next 要交付什么。
2. [项目与工具总览](../../development/tools.md)：AKEDatabase、VFS 浏览器、AnimeStudio、IL2CPP Dumper、C# Combat Spec 等分别做什么。
3. [证据链与研究方法](../../development/evidence.md)：一项游戏规则如何被发现、验证、复刻并进入生产代码。
4. [战斗系统研究结论](../../research/README.md)：当前已经串起的原生战斗架构、执行顺序、资源、技能、Buff 与伤害模型。
5. [Endaxis Next 架构](../../architecture/README.md)：新版代码分层、领域模型、编译与模拟流水线、项目格式、UI 边界。
6. [当前状态与路线图](../../handoff/current-context.md)：当前分支、近期提交、已完成能力、AKEDB 数据情况和后续优先级。
7. [操作与恢复手册](../../development/troubleshooting.md)：仓库路径、命令、服务、验证方式、远程环境和恢复任务时的检查清单。
8. [当前任务快照](../../handoff/current-context.md)：变化最快的短期状态，只用于快速恢复当前会话。
