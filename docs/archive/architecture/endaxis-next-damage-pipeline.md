...
场景应用流水线明确分为“项目编译 -> 环境能力预检 -> 执行已编译场景 -> 回执投影”。`executeCompiledScenarioSimulation` 是唯一的编译后执行阶段，专用环境入口只能在它之前装配和检查能力，不得复制时钟推进、结果冻结或资源曲线投影。当前保留标准伤害便捷入口，但它是能力配置适配器，不是第二套模拟引擎。
...
实体状态遵守“一场战斗、一个实体、一种状态一个所有者”：Buff、语义状态、元素附着和生命/失衡可以有不同生命周期对象，但同一种状态不能同时被装配层与伤害环境各保存一份。后续接入失衡时，应把敌人 `CombatVitals` 提升为场景共享实体状态并由环境交给装配根推进；不应继续向标准生命伤害适配器内部追加一套仅供伤害读取的失衡账本。
