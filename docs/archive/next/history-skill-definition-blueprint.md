# 历史：SkillDefinition 数据结构蓝图

> 历史记录：保留当时的设计、证据与验证范围。文中的“当前”“下一步”和机器信息不再代表现状；开发以文档总览和当前交接为准。

本文保留早期设计与推导，未逐条确认为当前实现。不要复制其中的接口作为开发依据。
当前说明见 [技能定义、编辑与执行](../../architecture/skills.md)，字段以公共类型为准。
...
时间轴上的每个放置块都有独立释放实例，但冷却属于一场战斗中的“干员 + 技能”身份。同一技能的
多个放置块必须共用一份冷却账本，且每帧只能推进一次；否则第二个放置块会错误地绕过第一次释放
留下的冷却。当前同一技能若出现互相矛盾的冷却配置会在装配时失败，后续自定义技能身份稳定后再
决定是否允许显式拆成不同技能。8. 到达 `costFrame` 时先尝试扣费，再执行该帧调度项。9. 扣费失败记录拒绝，但不删除时间轴动作。10. 所有调度项完成后自然结束；区间操作统一收到结束通知。
