...
首次了解当前架构，请先读 [事件系统：从游戏行为到程序结构](event-system-guide.md)。
本文件保留分阶段设计和迁移记录，含已被后续实现取代的历史描述，不宜按全文推定当前状态。
...
- 被动转换、校验、编译及注册仍只开放两个 Ability 事件。公共响应结构暂保留这个窄门禁，
  这是该历史检查点的实现边界，不是游戏领域约束。2026-09-10补充：samePriorityKey
  已从公共协议及全部消费删除；同优先级依注册顺序，不需要可交换标识。
- 2026-09-10：Buff 同事件/优先级分组已删除，每个原生序列独立注册和注销；
  无消费者的可交换响应推导也已删除，不应再抽象推广。
