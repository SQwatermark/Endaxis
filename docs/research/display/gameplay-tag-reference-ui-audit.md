...
来源、空标签和跨配置重复处理见[完整配置集恢复](../generation/gameplay-tag-config-set.md)。
...
- 完整配置集清单、各文件 SHA-256 和复现命令见[来源记录](../generation/gameplay-tag-config-set.md)；
- 正式生成器：`tools/game-data-compiler/scripts/generateGameplayTagCatalog.ts`；
- 生成目录：`src/data/combat/gameplayTagCatalog.generated.ts`；
- 时间膨胀配置与 Bundle 哈希见
  [`time-dilation-slot-and-curve-config.md`](../combat/time-dilation-slot-and-curve-config.md)。
