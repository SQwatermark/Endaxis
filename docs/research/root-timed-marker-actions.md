...
5. **显式维护表现边界**：`tangtang_ult` 已作为 Aura 图标时钟完整接入；`lizhiyan_attack4`、`lizhiyan_power_attack_effect` 仍可在确认没有数值消费者后按“UI/特效计时”忽略，并添加回归测试，避免未来数据变化后继续静默。

## 尚未闭环的边界

- 本文判断基于 1.4.4 本地 SkillData/BuffData 闭包；若后续版本新增通过代码硬编码读取 marker ID 的消费者，需要重新审计。
- `tangtang_ult` 的标记与两份 Aura Buff 现已整体贯通；它只覆盖图标倒计时，不改变 Aura Buff 的真实安装、清理或数值效果。
