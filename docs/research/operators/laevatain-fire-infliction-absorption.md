...
- 吸收投射物 `projectile_chr_0016_laevat_absorb` 的命中技能
  ...
  不能把同一规则扩大到含战斗动作的投射物。原生发射和回收仍然发生，因此模拟器保留投射物
  寿命及 `projectileLaunched` 事件。

本次运行时接线用最新 AKEDB `1.5.3@10024360-6` 复核。队伍监听 Buff、命中技能和投射物文件的
SHA-256 依次为 `8CF8F85A999C74D9CBEA0BDAED41FD471995F1C07FF5E632742779466ECA086B`、
`E23EC5A5BBD432B782C1DEEA6F19BD26B40CA4802855E8AC72FA2A644FA35FC9`、
`ED17F3AD7BD6729ADBCB03C80FAF83FD8003CA295A28BED2DA9CFCD02F10E126`。
...
- Buff 事件也可以执行 `LaunchProjectile`。这类序列与主动技能共用战斗层投射物调度器；不能因为
  回调只有表现动作，就让 Buff 运行时在发射步骤缺少调度器或跳过原生对象寿命。

## 验证

默认仓库生产回归使用 50 火抗木桩，构造四次“施加 1 层火焰附着 + 普攻末段伤害”，再在第 125
帧执行同构火焰探测命中。四层附着均以 `absorbed` 结束；天赋 1 级和 3 级分别把本次防御火抗
冻结为 40 与 30，三级探测伤害严格高于一级。该回归同时经过四次 Buff 事件投射物发射，确认
事件序列能够创建并回收投射物，不再因缺少调度器中断模拟。

莱万汀天赋 2 的原生根 Buff 只监听 `OnTakeDamage`；低于 40% 生命时才以 120 秒冷却创建周期治疗与
