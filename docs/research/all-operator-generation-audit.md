...
> 2026-08-31 当前口径：本文的 320/320 是源入口结构审计；统一 TS 正式产品门禁现为 **30/30 名干员、
> 310/310 个声明为主动的技能、30/30 份完整正式定义**。此前 `274/310` 与 `Tangtang 9/10` 是嵌套
> 投射物回调尚未闭合时的历史快照，阻塞已经解决。两套统计对象不同，仍不可互相替代。
...
- 技能入口：320 个。
- 进入严格中间层：320 个。
- 在正式 manifest 身份上下文中进入通用 DSL：320 个。
- 当前干员入口全可编译：30 名。
- 当前技能入口调用图中已结构化的区域持续动作引用：21 个。
- 当前技能入口中已结构化的事件监听器：17 个。

这里的“全可编译”仍采用保守口径：入口来自成长表，动作使用统一 resolvedSequence 编译器，
不为审计新增逐技能忽略项。跨技能换槽会复用正式 manifest 的稳定 key、完整技能组和已证明关系；
这只提供编译所需身份上下文，不把角色数值或规则重抄进审计器。某名干员的隐藏工件缺失时，
该干员会整体回退到逐入口审计，不能用残缺 sibling 集合猜测换槽。

## 分干员结果

| 干员           | 角色 ID             | 入口 | 已解析 | 已编译 | 全可编译 |
| -------------- | ------------------- | ---: | -----: | -----: | -------- |
| Perlica        | `chr_0004_pelica`   |    9 |      9 |      9 | 是       |
...
| Antal          | `chr_0023_antal`    |    9 |      9 |      9 | 是       |
| Tangtang       | `chr_0027_tangtang` |   11 |     11 |     11 | 是       |
| Pogranichnik   | `chr_0029_pograni`  |   10 |     10 |     10 | 是       |
...
| Liino          | `chr_0035_liino`    |   12 |     12 |     12 | 是       |
...
```powershell
# 历史命令：旧 Python 审计器已退役；当前使用 npm run audit:game-data:operator-active-skills
```
