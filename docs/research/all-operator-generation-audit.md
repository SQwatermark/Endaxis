...
| 阻塞类别                    | 技能数 |
| --------------------------- | -----: |
| `condition-main-operator`   |     62 |
| `buff-source-or-target`     |     51 |
| `projectile-child-actions`  |     44 |
| `damage-type-alias`         |     34 |
| `source-data-missing`       |     21 |
| `parser-channeling`         |     11 |
| `condition-entity-count`    |     10 |
...
优先级上，元素枚举别名属于低风险 parser 补全；Buff 来源/目标和投射物子技能副作用是覆盖面更大的
通用编译问题。主控身份条件虽出现最多，但必须先确认其分支副作用，不能统一常量折叠。
