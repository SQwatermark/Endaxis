...
| 3      | `attachSkill`        |     28 | 隐藏被动 SkillData 与 Buff 已纳入解析和逐项审计；当前真实样本仍被事件、光环、复杂 Buff 或原生属性消费端阻塞     |
...
阻断其他干员生成，也不会把失败定义写入正式运行时目录。当前清单中的真实候选均仍有至少一项阻塞，
因此本轮没有把任何隐藏被动误接入正式模拟：
...
对应事实与阻塞原因位于各干员 `*.audit.json` 的 `passiveSkills`。合成测试覆盖了可表达子集的完整输出：
隐藏被动黑板按养成等级解析，启用序列按原生顺序施加内联 Buff，并显式传递 Buff 黑板参数。
