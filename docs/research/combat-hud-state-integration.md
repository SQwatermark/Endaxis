...
- 干员轨道头已显示光标帧的终结技能量、当前施放名称以及冷却/连携候选数量；已有 Buff 节点分流
  继续复用同一光标帧。
- 战技/终结技按钮现由 `playerActionRoutes` 找到原生技能槽，再由光标快照中的 `skillSlots` 取得当前
  技能；运行时替换因此能改变按钮名称，但技能库分组完全不参与模拟或槽位选择。冷却遮罩只消费
  已有冷却区间；仍开放到模拟末尾、无法证明总时长的区间不会伪造百分比。
- 技能按钮 Buff 进度已按原生事件指针和 Buff 自身时钟进入统一快照；不能从 Buff 名称、图标、技能库
  分组或时间轴段宽度反推。

### 技能按钮进度的新增证据边界

后续定点反编译确认原生没有“冲突优先级”：`SkillButton._OnBuffIconChange` 在 Buff 施加时用最后一个
带对应显示标记的 Buff 覆盖当前指针；结束事件只有与当前指针实例相同才会清空，清空后不会回退到
仍存活的旧 Buff。普通技能弱进度只改变样式。快照现已按回执顺序复刻这套指针语义。

数值进度不能从时间轴段宽度线性推算。原生每次刷新读取 Buff 的 `lifeTime / duration`；对象时间膨胀、
Buff 暂停或动态改时长都会令“现实帧占已知结束帧的比例”不等价。因此当前快照先保留当前 Buff 身份和
弱样式，不能为了填充遮罩而制造近似值。

现已增加统一 `BuffProgressCurve`：模拟只为带原生 HUD 进度标记的 Buff 逐帧采样实际剩余
寿命比，并在结果冻结时删除共线中间点。普通倒计时通常只留下起止点；暂停、对象时间膨胀或动态时长
导致的斜率变化会保留拐点。HUD 按现实光标帧在这些点之间采样，因此没有把所有对象假设成统一模拟
速度，也没有增加逐帧回执噪声。

该曲线现已泛化为 `BuffProgressCurve`，同时覆盖 `showProgressInHpBar`。原生主控生命栏不是简单采用
最后施加 Buff：它先倒序选择当前主控持有的最新候选；没有时保留仍有效的当前指针；当前指针失效后
才回退到全队候选末项。Next 按主控时间线与 Buff 回执重放同一状态机，并只在当前主控轨道头显示
独立进度条。数值仍采样 Buff 自身 `lifeTime / duration`，原生平滑动画不进入战斗状态。

### 架势结点与多段弱点的边界

原生 `UIPoiseBar._TryInitPoiseKnot` 从控制器读取归一化结点阈值，并按最大架势值配置固定结点。Next
因此直接在架势条上显示 `scenario.enemy.editable.stagger.knotThresholds`；当前填充仍由架势曲线取样，
没有建立证据不足的“逐结点击破”账本。原始阈值表示已损失比例，状态头的条宽表示剩余比例，因此
显示坐标使用 `1 - threshold`，不能直接照抄阈值。

进一步反编译 `UIPoiseBar.OnPoiseChange`（RVA `0x03CED1D0`）确认，它用当前已损失比例与旧的 UI fill
跨越阈值时调用 `UIPoiseKnot.KnotBreak`（RVA `0x0B1316B8`）；后者只播放动画。恢复越过阈值不会调用
同一入口。因此时间轴光标快照只保留固定刻度，不把一次性的播放效果虚构成持久“已击破结点”状态。

`UIHeadBar` 的 `UIWeaknessNode` 是与 `UIPoiseBar` 并列且互斥显示的另一组件。其激活、总格数和当前格数
来自 `SetMultiTimesWeakness` / `SetWeaknessAction` 驱动的敌人弱点窗口，不是元素弱点或抗性。当前木桩
模型不执行敌人主动行为，所以不伪造这套窗口；将来只有显式外部事件进入模拟时才可显示。
