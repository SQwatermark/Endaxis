...
绝对路径、分支、未提交状态、服务、连接信息与本机可用文件不得写入共享交接文档；如需持久化，
复制[本地环境模板](local-environment.example.md)到`.local/handoff/environment.md`后填写。
新会话必须用`pwd`、`git status`、`git worktree list`和实际文件检查确认环境。

## 1. Endaxis

- 开发工作树与分支：以当前机器的实际仓库状态为准，不能仅凭目录名Endaxis选择。
...
- 路径、分支与服务状态：以当前机器的实际检查为准；历史端口不等于当前已运行。
...
- 路径、分支与远端：以当前机器的实际检查为准；不要从历史文档推断当前origin。
...
主机地址、用户、游戏安装路径、端口和服务状态属于机器本地配置，只维护于被忽略的`.local/`。
共享文档可以记录带版本和哈希的证据来源，但不得把一次连接结果写成另一台机器的当前状态。
