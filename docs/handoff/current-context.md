...
- Python 生成器测试：269 项通过；敌人 rank 提取器测试：2 项通过；能力实体提取器测试：2 项通过；生成器 `--check` 通过；
...
2. 在已有实体时间膨胀执行器之上建立干员级匹配生成点闭包，优先评估 Li Zhiyan 连携技的 owner/tag 查询；只有所有可能命中的生成点都已动态迁移时才开放正式输出；
