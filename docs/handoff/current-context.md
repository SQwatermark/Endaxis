...
- 已通过生成器类型检查、窗口策略单测、主动技能渲染单测和伊冯默认仓库实际放轴/模拟定向回归。
  停止点没有继续跑完整 Next、生产生成器类型检查，也没有批量重生成其余 29 名干员。
- 下一接手顺序：先跑完整类型与回归门禁；再批量审计所有多段基础攻击的旧/新块宽并原子重生成，
  确认没有把换槽技能集合误当连段；然后回到三条真实轴的逐 cast 差分。HUD 原生规则等台式机证据，
  不要在当前机器上猜。
