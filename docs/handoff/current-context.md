...
- 新增生产仓库级逐项审计：30 名干员的基础技能和变体共 **301 个**，每个都用
  ...
  **286/301** 可完整模拟；其余 **15** 个已按精确技能身份和错误文本锁定为已知失败。
  新失败不能静默加入清单，旧失败修复后也会反向使门禁失败，迫使删除过期豁免。
- 15 个失败按原因分为：7 个缺动作/实体黑板初值，2 个梨诺技能替换与已放置链冲突，
  1 个梨诺未注册递归 Buff，2 个能力实体目标组，1 个能力实体 Buff 目标，
...
- 下一顺序：先从有明确数值影响且可以取证的 15 个失败开始消除，同时关闭
