...
- 本文更新前 `npm run type-check:next` 通过；Next 全量 Vitest 为 177 个文件、1123 项全部通过。新增回归覆盖对象局部时间推导的技能实际宽度与命中点、块外延迟命中、未决宽度判定、零宽展示技能边界、时间膨胀实际区间裁剪，以及既有 Buff、资源、状态、能力实体和干员转换链。
