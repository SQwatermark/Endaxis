...
- `npm run test:next`：169 个文件中 168 个、963 项中 961 项通过；仅余既有 `runStandardPlayerDamageScenarioSimulation.test.ts` 两项时间线停滞。
