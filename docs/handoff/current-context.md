...
- 正式生成套装从 2/23 扩大到 8/23：新增 `suit_agi01`（敏捷 +50、满血物理增伤 20%）、
...
- 当前门禁：公共编译器 37 文件 / 174 项、Next 208 文件 / 1504 项及两套类型检查通过；
...
- 原生 `suit_atk01` 取代 `aburreys-legacy`，旧 slug 反向 alias 到新定义；其余未转换原生套装仍指向
  ...
  8/23。剩余类别为 `OnOutputBuff`、暴击/附着事件、Aura 和少量公共条件动作。`tmp/` 继续不提交。
