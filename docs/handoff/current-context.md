...
- 证据边界：GlobalBuff 身份清单已由同批 31 名候选反向证明没有漏项；GameplayTag 使用本次已
  ...
  直接注册。
- operators 与 common-buffs 候选阶段已经接入统一重建。下一步直接做候选虚拟落位类型检查、31 名
  所有可放置技能单放与组合轴模拟、Typhoeus 体积审计和资源引用门禁；全部通过后才设计原子发布，
  不再重复来源下载或零散复制生成文件。
...
- 本轮按用户要求停在候选发布门禁之前：尚未实现候选虚拟落位类型检查，也尚未对 31 名候选执行
  新一轮全技能/组合轴模拟。不要把正式目录现有 30 名测试结果误记成这批 31 名候选已经通过。
