...
- Buff 轴使用原生图标、自动推导的 i18n 名称和旧版灰色条样式；过量时由松散布局增高轨道而非压缩或
  裁切。能力实体仍与 Buff 分属独立领域模型，只有定义明确声明的 `timelinePresentation` 才投影到状态栏；
  莱万汀熔火等聚合展示必须由真实 Buff/实体回执和显式展示元数据驱动，不能把所有实体一律当 Buff。
...
- 本轮定向测试 120/120 与追加的编辑器 64/64、Next `vue-tsc` 通过。全量 Next 回归仍暴露原有六项
  ...
  `spawn UNKNOWN` 导致两个 worker 未启动。改用 thread 池后完整收集 3833 项，其中仍是原有六项断言
  失败，另有六项重型模拟在并发争用下超过 5 秒；不能把环境超时当作本轮编辑器回归。生产构建通过，
  仅保留既有的混合动态/静态导入与大 chunk 警告。
