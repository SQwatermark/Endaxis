...
- 庄方宜条件投射物链已严格闭环：`abilityEntityHits` 必须与根条件的投影实体在数量、绝对帧、完整载荷和身份上逐项相等才按重复表示处理，实际生成仍留在精确条件分支；空子战斗 SkillData 作为惰性逻辑实体生成，`abilityEntitySkillId` 为空的假目标模板也生成但不附造 `childSkill`。普通战技及强化战技由此通过通用 DSL，全量提升到 320/320 可解析、298/320 可编译、14 名完整直转。庄方宜剩余阻塞是终结技 `Context/ult_postmodel_mirror` 与强化三段普攻 `Context/thunder` 的 Buff 目标来源；Tangtang 仍停在 Aura 进出生命周期。
- 本轮门禁：生成器 345 项通过；全量生成与 `--check`、`npm run type-check:next` 通过；Next Vitest 178 文件、1128 项全部通过。`tmp/` 仍仅为未跟踪审计输出，不得提交。
