...
644 MiB 降到约 445 MiB；生产时间轴入口由约 3.97 MB 降到 2.73 MB，入口 CSS 由约
200.7 kB 降到 134.8 kB。伤害分析、干员构筑和干员定义编辑器已经逐项打开验证。
...
2. **定义按需加载**：生产页面内存已测，关闭对话框的依赖也已移出首页。主页面仍同步装配全部游戏
   定义；若继续优化，应建立“轻量选择目录 + 当前项目完整定义”的两层加载入口，并在建立项目会话
   前完成所需定义加载，不能把 Promise 查询侵入同步模拟核心。
