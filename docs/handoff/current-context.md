...
- 桌面 `GameAssembly.dll`（SHA-256 `0C5573679BC6DEC2D068A14335466DB7CCF20AF9BAE2B983FB9D45677D80FFCE`）的静态反汇编进一步确认：`FinishOwnerAction.ExecuteInternal` 解析目标后按 Entity `ObjectType` 分派；`AbilityEntityInfo.type` 原生返回 `0x200`，走通用完成路径，而不是 `0x20` 的 Release 或 `0x40` 的投射物路径。Fluorite 表面上的 90 帧结束后仍有 149 帧伤害并非反证：子技能在 0–89 帧还有两条 `JumpToAction`，分别以目标死亡跳到 89、目标持有终结技 Buff 跳到 149，两个结束属于替代路径。Lifeng 终结技也在局部 67 帧以 `isCombo == 0` 跳到 150，跳过后续 121 帧动作。`JumpToAction` 在首次执行时立即检查直接条件，此后在动作存续期间每 Tick 重试，首次成功后置位并只调用一次宿主 `Skill.JumpTo`；空条件立即成功。原生 `TimelineActionProcessor.JumpTo` 对起始时间严格早于目的帧的待执行序列调用 `SequenceAction.JumpToEnd`，后者逐个把尚未执行的子 Action 标为结束；已开始且结束时间不晚于目的帧的序列会 End（动画序列另有下一帧完成队列），跨越目的帧的活动序列保留，起始时间恰等于目的帧的序列不被跳过。Next 已新增严格向前的 `TimelineActionProcessor.jumpTo` 调度原语与对应测试；条件重试、技能/实体局部时钟改写和执行中重入仍未闭环。当前 1.4.4 公共 SkillData 共找到 436 条跳转、其中 413 条启用；297 条没有直接条件，其余条件和外层容器形状很多，不能用两个干员样本概括。生成审计因此新增精确 `actionPath`：Fluorite 两条是根序列动作，Lifeng 则明确定位到 `IfElseAction.succeedActions`，可与既有条件树关联，不再把“直接条件为空”误读成无条件跳转。因此所有带跳转的能力实体子图继续严格阻塞，Fluorite 保留父时间轴 99/159 帧投影，Lifeng 退出此前的子时间轴内嵌。“直接结束必须是线性末端动作”的守卫仍作为独立防线。生成器还会递归把这类未建模跳转自动写入干员 `conversionSupport.skillBehavior`；Fluorite 的 `battleSkill` 不再被静默标成已支持。
...
- Python 生成器测试：266 项通过；敌人 rank 提取器测试：2 项通过；能力实体提取器测试：2 项通过；生成器 `--check` 通过；
...
- `npm run test:next`：169 个文件中 168 个、979 项中 977 项通过；仅余既有 `runStandardPlayerDamageScenarioSimulation.test.ts` 两项时间线停滞。
