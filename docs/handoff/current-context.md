...
- 1.4.4 全量正式组装为 220/220 成功、0 blocked：65 件护甲、56 件手套、99 件配件，共 610 条
  可见词条、647 个正式修正，其中 403 个四维属性、98 个面板属性、137 个伤害倍率、9 个治疗输出修正。
...
- 当前门禁：`type-check:game-data`、`type-check:next` 通过；游戏数据编译器 30 文件 151/151，Next
  204 文件 1484/1484。下一步应把这 220 份结构化定义接入确定性渲染、正式仓库注册和生产模拟扫表，
