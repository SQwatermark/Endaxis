...
- 复合状态工厂从“只有目录和独立单测”接入标准模拟：异类附着会把真实消耗层数交给对应工厂，按
  SkillSetting 列和来源法术增强属性求值，再把工厂输出黑板交给最终状态。当前首先闭环三种“异元素
  → 电”的导电定义；自然→电一层会得到 12 秒、12% 法术增伤，2–4 层对应 18/24/30 秒和
  16%/20%/24%。三种导电 Buff 均保留原生图标、Tag、叠加身份和全元素法术伤害过滤。
- 当前边界：导电状态的持续增伤已进入后续伤害公式；其创建瞬间的异常初始伤害尚未接入外部 Buff
  定义动作端口。其余 9 种异元素组合也仍需按最终 Buff 真实动作树逐族接入，不能用空状态定义消除
  报错。下一步应先补通用“Buff 生命周期伤害”端口，再完成燃烧、冻结、腐蚀与全部初始异常伤害。
