...
收尾更新：技能执行程序已与时间轴分组/等级/块宽解耦；原生 CastData.CostData 的来源读取
也已在技能施法与伤害费用间去重。后者只统一来源结构，不合并消费规则。正式投射物回调尚未
传入完整施法元数据，也未换成独立 AbilitySystem/SkillRuntime，临时宿主仍是首要余项。

补充：技能 JumpTo 超自然时长已按原生接同步结束，并共用重入门禁；普通技能/临时回调
均消费同一实现。118文件1455项战斗回归及四轴完整结果通过。仍未关闭完整回调宿主项。

最新正式迁移：duration-finish 回调已从即时 body 改为完整 callback 程序，保留多区间、
共享黑板和自然结束，汤汤重新生成。仅动作程序层完成，独立 AbilitySystem 事件归属、
所有发射引用及其他即时投影仍未完成；细节与验证见 current-context 最新节。
