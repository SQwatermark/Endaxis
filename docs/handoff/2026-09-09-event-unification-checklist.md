...
2026-09-10 更新：当前实例结束来源投影与点燃发布归属已接通。消费保护不是禁止结束，
目前仅见未接入敌人数据使用，不在本轮新增能力；结束/消费/吸收事件 Buff 黑板已从快照
迁移到实际实例上下文，其他子类与目标门禁仍待核对。具体证据见 combat-spec/event-buff-blackboard-instance。
下方按时间追加的记录不是最新验收清单；整体仍未完成。
...
通知自己的来源仍需审查；不是整个 P3 已完成。1170 项运行时/Buff 测试通过，四条
真实轴全回执/诊断一致，详细边界见 current-context 最新条目。

最新：临时监听 addedBuff/outputBuff 已通过公共 abilityEvent 触发器直订阅，公共
定义、校验、编辑器、转换、运行时及正式4份干员8处入口闭环。旧手写别名兼容尚存，
正式干员生成不再使用。潜能0/5全量审计、四条真实轴、1447项扩展回归及应用类型
检查通过。此前“临时事件身份待迁移”的条目为历史；其余事件/上下文边界仍未完成。
...
后续武器消耗 Buff、物理异常也已迁移，生成器不再输出旧语义触发器；10份正式武器
仅事件入口变化，四条真实轴回归一致。剩余主线是技能临时监听的兼容入口及优先级/上下文边界；
只收束已有技术债，不扩展新的游戏机制。

临时监听阶段已依据反编译纠正：EventListenerAction 与 Ability.Enable 走同一动作
容器，生成 dataAction/0 并共享非默认优先级拒绝校验。新增顺序/注销回归，定向176项
通过；31干员潜能0/5审计与四条真实轴回归通过，5份干员仅阶段/优先级字段变化并落位。
仍待统一其语义身份适配和序列遍历，不能将此项记录成临时监听全面迁移完成。

随后序列遍历已共用 compileAbilityEventPrograms，空结束回调与来源下标有回归保护；
151项定向通过，31干员重生成全部不变。当前临时监听剩余项为语义身份适配，
不再重复实施阶段修正、优先级校验或程序遍历迁移。
...
- [ ] BuffEnhanceChanged 的原生 Buff 实例/来源上下文尚不完整；不能借宿主补齐，
      ...
      结束/减层的 C# 载荷及 Endaxis 四条 ID/Tag 动作路径已修正，实例引用已保留；
      正向增强与其他直接结束入口仍待来源核对，不重复实现已完成路径。
- [x] storeShieldValueCurValue 的事件快照与原生显式 Target 当前有限护盾读取
      不等价；2026-09-10 已贯通公共目标、转换、运行时实时读取及编辑校验。
      GainedValue 仍读147事件，缺目标/所需事件成功不写；未开放未经审计的目标种类。
