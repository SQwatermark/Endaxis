...
| `baseStats` | object | 基础属性成长表 |
| `talents` | TriggerEffect[][] | 天赋效果（每组对应一个天赋等级） |
| `potentials` | TriggerEffect[][] | 潜能效果 |
| `skills` | object | 技能定义（重击/闪避/下落攻击/处决/战技/连携/终结技） |

### 武器（Weapon）

5 类武器 × 多个稀有度，总计约 81 把：
...
23 个套装，当装备同一套装的 2 件或 4 件时激活套装效果。套装效果通过 `TriggerEffect` 定义触发条件和效果。
