...
### 不一致 4：套装门槛用 3 件而非 2/4 件

```typescript
if (count < 3) continue;  // L264
```

游戏内套装效果通常是 2 件和 4 件两档，但代码用 `count < 3` 判断，意味着 2 件的套装效果不会被激活。这与游戏实际机制不符——要么是游戏就是 3 件触发（可能），要么是计算有误。
...
effects: h.effects.map(e =>
  resolveEffect(e.id ? e : { ...e, id: uid() }, idx)
)
```
...
### 2. `collect.ts` 内部 9 个不一致点

详见上文各节。优先修复：
- `hydrateEffect` 调用统一化（所有效果源走同一入口）
- `effect.name` 语义统一（全部存 locale-agnostic 键，渲染时查表）
