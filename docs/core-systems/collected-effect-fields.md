...
```typescript
{
  effect: {
    kind: 'status',
    id: 'laevatain_talent0_effect1',     // ← generateUniqueId 生成
...
**值**：字符串，如 `"laevatain-melting-flame"`、`"laevatain_talent0_effect0"`、`"k3x8m2p"`（随机）
...
| 3 | `uid()` 随机字符串 | `k3x8m2p` | ✗ | 嵌套 hit effects 不经过 `ensureEffectId` |

sheet 定义的 id（如 `laevatain-melting-flame`）有业务含义 — `AppendEffect.targetEffect`、`DerivedEffect.sourceEffect`、`PatchEffect.targetEffect` 都用这些 id 做跨效果引用。`makeEffectId` 生成的是纯位置性标识，不会被其他效果引用。`uid()` 是非确定性的，同一配置两次 collect 会产出不同 id。（详见 `collect-analysis.md` 不一致 8、9）
