...
所有可见的 segment 从 `OPERATOR_EFFECT_APPLY` / `OPERATOR_EFFECT_EXPIRE`、`ENEMY_EFFECT_APPLY` / `ENEMY_EFFECT_EXPIRE` 等运行日志重建。第 3 类不产生普通可见 segment，只影响面板数字；其中部分初始状态会以隐藏初始效果形式进入运行时，用于计算属性。

**例外**：连携窗口（comboWindow）不走 `TimelineBuffLayer`——有独立的 `projectComboWindows.ts` 投影和 `TimelineComboWindowBar.vue` 渲染。它从 `OPERATOR_EFFECT_APPLY`/`OPERATOR_EFFECT_EXPIRE` 事件中提取 `id` 以 `combo-window` 结尾的事件，生成金色虚线 bar 在轨道底部。冷却判断通过新增的 `comboNotOnCooldown` condition 在运行时直接查询 `ActorState.actor.getCdReduction()`。
