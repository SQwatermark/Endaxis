...
| 事件类型 | 处理器 | 说明 |
|---|---|---|
| `ACTION_START` | ActionStartHandler | 动作开始：消耗技力/终结技能量，应用 triggerWindow 效果 |
...
| `ULT_ENERGY_CHANGE` | UltEnergyHandler | 终结技充能变化 |
| `STAGGER_CHANGE` | StaggerChangeHandler | 失衡值变化 |
| `EFFECT_START` | OperatorEffectHandler / EnemyEffectHandler | 效果开始（附着/爆发/状态施加） |
| `EFFECT_END` | — | 效果结束 |
| `REACTION` | effectDispatch | 元素反应触发 |
