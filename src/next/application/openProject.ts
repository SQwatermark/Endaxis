/**
 * 存档进入应用状态前的统一打开边界。
 * 这里负责组合格式解析、定义引用校验与游戏数据版本判定，但不会自动执行数据迁移，
...
/** 数据迁移执行后的结果；迁移实现必须由调用方显式提供，打开入口不会自行构造。 */
...
interface RevisionMismatchResultBase {
  ...
  /**
   * 使用目标数据校验得到的引用问题，仅用于帮助迁移实现判断影响面。
   * revision 不匹配时这些问题不能单独证明项目损坏。
   */
  indexIssues: readonly ValidationIssue[];
}
...
/**
 * 打开不可信项目输入，并判定它能否安全使用当前游戏数据。
 * 定义引用校验会先执行；版本不匹配时其问题只作为诊断返回，不会掩盖版本边界。
 */
export function openProject(
  ...
  const indexIssues = definitionValidation.ok ? [] : definitionValidation.issues;
