...
export function formatSimLogEntry(entry: SimLogEntry, opts?: FormatSimLogEntryOptions) {
  ...
  switch (entry.type) {
    case "ACTION_START": {
      ...
      return withPrefix(entry, `action start ${skillId} (${typeLabel}) id=${actionId}${extra}`, opts);
    }
    case "ACTION_END": {
    ...
    case "DAMAGE_HIT": {
      const { sourceId, targetId, stagger, hitData, actionId } =
    ...
    case "ULT_ENERGY_CHANGE": {
      ...
      return withPrefix(
        entry,
        `${actorId} gauge ${sign}${change} id=${sourceId}`,
        opts,
      );
    }
    case "SP_REGEN_PAUSE": {
