...
import { validateActionSequenceDefinition } from '../../../core/game-data/validation/actionPrograms';
...
} from '../../../core/combat/buffs/buffOperationExecutor';
import { BuffDefinitionOperationTarget } from '../../../core/combat/buffs/buffDefinitionOperationTarget';
import { ActionBlackboardOperationExecutor } from '../../../core/combat/actions/actionBlackboardOperationExecutor';
...
import { RuntimeTargetContext } from '../../../core/combat/abilities/runtimeTargetContext';
...
const ultimate = 'abilityentity_chr_0012_avywen_ultimate_skill_lance';
...
describe('回收标记作用于真实枪实例，而非木桩', () => {
  it.each([0, 1].flatMap(index => [0, 3].map(count => ({ index, count }))))(
    ...
    async ({ index, count }) => {
      const base = gameDataRepository.getOperator('avywenna')!;
      ...
      const result = await new ScenarioSimulationService({
        index: {
          ...gameDataRepository,
          getOperator: slug =>
            slug === operator.slug ? operator : gameDataRepository.getOperator(slug),
        },
        resources: {
