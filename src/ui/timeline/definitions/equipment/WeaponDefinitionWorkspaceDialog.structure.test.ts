...
import buildDialogSource from '../../library/WeaponBuildDialog.vue?raw';
...
describe('WeaponDefinitionWorkspaceDialog structure', () => {
  it('derives a project template and atomically switches the equipped instance', () => {
    expect(coordinatorSource).toContain('deriveProjectWeaponTemplate');
    expect(coordinatorSource).toContain('switchTrackToCompatibleWeaponTemplate');
    expect(coordinatorSource).toContain('replaceProjectWeaponTemplateDefinition');
  ...
  it('定义保存等待场景 watcher 标脏后只启动一份新模拟', () => {
    ...
    expect(coordinatorSource).toMatch(
      /function saveWeaponDefinition[\s\S]*?options\.onDefinitionChange\(\);[\s\S]*?function resetWeaponDefinition/,
    );
