...
function castHitMarkers(trackIndex: TrackIndex, castId: string): TimelineHitMarkerView[] {
  ...
  if (simulationRun.value !== null) {
    const projectedByStepKey = new Map(
      castModel.hitMarkers.map(marker => [marker.stepKey, marker] as const),
    );
    const triggeredCountByFrame = new Map<number, number>();
    return (hitOccurrences.value.get(castId) ?? []).map(hit => {
      const projected = projectedByStepKey.get(hit.stepKey);
      const triggered = projected?.triggered === true || projected === undefined;
      const triggeredStackIndex = triggeredCountByFrame.get(hit.frame) ?? 0;
      if (triggered) triggeredCountByFrame.set(hit.frame, triggeredStackIndex + 1);
      return {
        stepKey: hit.stepKey,
        hitId: hit.hitId,
        executionFrame: hit.frame,
        leftPx: timelineFramePx(hit.frame) - timelineFramePx(publishedStartFrame),
        // 运行时出现、却不在技能根定义中的命中同样属于追加触发命中。
        triggered,
        ...(triggered ? { triggeredStackIndex } : {}),
  ...
  return castModel.hitMarkers
    ...
    .map(marker => ({
      ...
      triggered: marker.triggered === true,
