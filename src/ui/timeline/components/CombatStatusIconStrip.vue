...
const items = computed(() =>
  props.indicators
    ...
    .map(indicator => {
      const durationEndFrame = indicator.durationEndFrame ?? indicator.endFrame;
      const duration = durationEndFrame - indicator.startFrame;
      const remainingRatio =
        indicator.hasFiniteLifetime === true && duration > 0
          ? Math.max(0, Math.min(1, (durationEndFrame - props.frame) / duration))
          : null;
      ...
      const icon = indicator.iconPath ?? getIconAssetPath(indicator.iconId);
      return {
        ...indicator,
        ...
        detail: {
          ...
          ...(indicator.startReason === undefined ? {} : { startReason: indicator.startReason }),
          ...(indicator.endReason === undefined ? {} : { endReason: indicator.endReason }),
          ...(indicator.stackingType === undefined ? {} : { stackingType: indicator.stackingType }),
          ...(indicator.parentBuffId === undefined ? {} : { parentBuffId: indicator.parentBuffId }),
          icon,
