...
<template>
  <div class="event-trigger-grid">
    <label>
      <EditorFieldLabel
        :label="t('timeline.skillEditing.triggerKind')"
        :help="t('timeline.skillEditing.fieldHelp.eventTrigger')"
      />
      <select
        :value="event.kind"
      ...
      >
        <option v-for="kind in KINDS" :key="kind" :value="kind">
          {{ t(`timeline.skillEditing.triggerKinds.${kind}`) }}
        </option>
      </select>
    </label>
    <label
      ...
      "
    >
      <span>{{ t('timeline.skillEditing.triggerScope') }}</span>
      <select
        :value="event.scope"
      ...
      >
        <option v-for="scope in SCOPES" :key="scope" :value="scope">
          {{ t(`timeline.skillEditing.triggerScopes.${scope}`) }}
        </option>
      </select>
    </label>
    <label v-if="event.kind === 'operatorHealed'">
      <span>{{ t('timeline.skillEditing.eventRole') }}</span>
      <select
        :value="event.role ?? ''"
        @change="setHealRole(($event.target as HTMLSelectElement).value)"
      >
        <option value="">{{ t('timeline.skillEditing.eventRoles.target') }}</option>
        <option value="source">{{ t('timeline.skillEditing.eventRoles.source') }}</option>
      </select>
    </label>
    <label v-else-if="event.kind === 'buffConsumed'">
      <span>{{ t('timeline.skillEditing.buffIds') }}</span>
      <textarea
        :value="event.buffIds?.join('\n') ?? ''"
        :placeholder="t('timeline.skillEditing.anyValue')"
        @input="setBuffIds(($event.target as HTMLTextAreaElement).value)"
      />
    </label>
    <template v-else-if="event.kind === 'spGained'">
      <label>
        <span>{{ t('timeline.skillEditing.spGainSource') }}</span>
        <select
          :value="event.source ?? ''"
          @change="setSpGainSource(($event.target as HTMLSelectElement).value)"
        >
          <option value="">{{ t('timeline.skillEditing.anyValue') }}</option>
          <option v-for="source in SP_GAIN_SOURCES" :key="source" :value="source">
            {{ t(`timeline.skillEditing.spGainSources.${source}`) }}
          </option>
        </select>
      </label>
      <label>
        <span>{{ t('timeline.skillEditing.spGainKind') }}</span>
        <select
        ...
        >
          ...
          <option v-for="kind in SP_GAIN_KINDS" :key="kind" :value="kind">
            {{ t(`timeline.skillEditing.spGainKinds.${kind}`) }}
          </option>
        </select>
      </label>
    </template>
    <label v-if="event.kind === 'damageTagHit'">
      <span>{{ t('timeline.skillEditing.damageTag') }}</span>
      <select
        :value="event.tag"
        @change="
      ...
      >
        <option v-for="tag in DAMAGE_TAGS" :key="tag" :value="tag">
          {{ t(`timeline.skillEditing.damageTagNames.${tag}`) }}
        </option>
      </select>
    </label>
    <label v-else-if="event.kind === 'elementalInflictionApplied'">
      ...
      >
        <option v-for="element in DAMAGE_ELEMENTS" :key="element" :value="element">
          {{ t(`timeline.skillEditing.damageTypes.${element}`) }}
        </option>
      </select>
    </label>
    <label v-else-if="event.kind === 'physicalInflictionApplied'">
      <span>{{ t('timeline.skillEditing.physicalInflictionType') }}</span>
      <select
      ...
      >
        <option v-for="type in PHYSICAL_INFLICTION_TYPES" :key="type" :value="type">
          {{ t(`timeline.skillEditing.physicalInflictionTypes.${type}`) }}
        </option>
      </select>
    </label>
    <label v-else-if="event.kind === 'skillHit'">
      <EditorFieldLabel
        ...
        :help="t('timeline.skillEditing.fieldHelp.eventSkillGroupKey')"
      />
      <input
        type="text"
    ...
    <template v-else-if="event.kind === 'statusExpired' || event.kind === 'statusConsumed'">
      <label>
        <EditorFieldLabel
          ...
          :help="t('timeline.skillEditing.fieldHelp.eventStatusKey')"
        />
        <input
          type="text"
