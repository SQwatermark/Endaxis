...
const activeSetBonuses = computed(() => {
  ...
  return Array.from(counts.entries())
    ...
    .map(([setSlug, equippedCount]) => {
      const description = getGearSetGameBonuses(setSlug, locale.value);
      const setName = getGearSetGameName(setSlug, locale.value);
      return { setSlug, description, setName, equippedCount };
    })
